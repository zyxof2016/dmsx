...
- **范围**：权限绑定在平台级 `roles` 与租户级 `tenant_roles`。API 层校验 **URL 路径中的 `{tenant_id}` 必须落在 JWT 允许的租户集合内**（`tenant_id` ∪ `allowed_tenant_ids`）。**RBAC**：租户路径请求优先使用 **`tenant_roles`** 按活动租户覆盖；无键时回退令牌级 **`roles`**，并可叠加租户内绑定/自定义角色。平台路径请求（`/v1/config/...`、`/v1/tenants`）仅使用令牌级 **`roles`**，不会套用租户绑定或租户自定义角色；租户自定义权限名不包含 `platform.read` / `platform.write`。切换租户即换 URL 前缀；签发方维护 `allowed_tenant_ids` 与 `tenant_roles`。（完整约定见 [`API.md`](API.md)。）
