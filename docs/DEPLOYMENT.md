...
批量部署建议：平台侧批量预注册并导入/导出 CSV，每行可包含 `registration_code`、`hostname`、`platform`；结果中导出 `enrollment_token`、`enrollment_uri` 与启动命令模板。设备侧可由脚本、MDM、工厂预置或扫码流程直接注入 `DMSX_DEVICE_ENROLLMENT_TOKEN` 启动 Agent，实现零接触注册。
