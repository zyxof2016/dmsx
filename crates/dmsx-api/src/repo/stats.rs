...
pub async fn get_stats(conn: &mut PgConnection, tid: Uuid) -> Result<DashboardStats, sqlx::Error> {
    let base = sqlx::query_as::<_, StatsRow>(
        "SELECT \
    ...
    let platforms = sqlx::query_as::<_, CountBucketRow>(
    ...
    let cmd_statuses = sqlx::query_as::<_, CountBucketRow>(
    ...
    let sev_counts = sqlx::query_as::<_, CountBucketRow>(
