...
```bash
# 控制面 HTTP API（含远程桌面 WebSocket 中继）
...
Agent 会自动：注册设备 → 定期发送心跳（含系统遥测）→ 轮询并执行命令 → 收到 `start_desktop` 命令后采集屏幕并推流。
...
| `crates/dmsx-api` | 控制面 ApiGateway（Axum）— 含远程桌面 WebSocket 中继 + AI API |
| `crates/dmsx-device-gw` | 数据面 DeviceGateway（Tonic gRPC + Health） |
...
| 远程桌面 | ✅ | WebSocket JPEG 帧流 + 键鼠控制，浏览器内嵌，RustDesk 备选 |
